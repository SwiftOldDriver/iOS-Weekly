...
iOS11 所推出的 CoreML 让App开发者可以不用学习了解 Deep Learning ，不用亲自训练模型，拿训练好的模型就可以在 app 里快速使用。并且搭配开放了几个视频识别物体的模型 Object Detect，那么除了苹果开放的识别物体模型，现在还有哪些地方可以找到更多的训练好的 CoreML 模型呢？这里面介绍3个Link，里面有很多机器学习开发者训练好的有趣的模型，比如照片转油画/面部情绪识别/人物年龄猜测/文章分类/文字情绪识别等等，iOS开发者的福音，可以拿来做一些有趣的个人 idea App
