...
[@邦Ben](https://weibo.com/linwenbang)：众所周知，Swift 从 1 开始到现在 5，ABI 一直在不断的更迭，也已经趋于稳定，在 Swift 5 上稳定下来后，会带来一系列的优化。该文为优化提案，该提案中主要有两点：

- 为库增加 `library evolution` 构建模式，该模式在不破坏 ABI 稳定的前提下，对类型进行更改。
- 允许这类型的库不使用这种灵活属性，从而使用编译优化提高性能。
