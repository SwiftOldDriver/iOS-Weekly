...
[四娘](kemchenj.github.io)：ABI 稳定的进程还没有完全结束，Swift 5 里达成的是标准库的 ABI 稳定，但这种 Module ABI 稳定的机制还没有暴露给用户使用，目前标准库是通过一些私有的注解和编译选项来使用这个机制，所以这次的提案内容就是讨论如何将它作为一个功能暴露给用户。
...
[@邦Ben](https://weibo.com/linwenbang)：Mattt 大佬说：大量的调整图片的代码都宣称自己是最正确的。因此，Mattt 给出5种不同的技巧以供选择。什么情况我们需要缩小调整图片尺寸？当图片尺寸远大于需求的时候，缩小可以节省内存提高性能。
