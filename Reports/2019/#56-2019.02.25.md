...
4. 通过WKWebView 向 Html 注入 JavaScript 并调用 原生代码
...
本译文介绍了 `ExpressibleByStringInterpolation` 协议的三种扩展实现，以精确控制字符串插值的打印方式。`ExpressibleByStringInterpolation` 协议在 Swift 3 时被废弃，如今被以一种更优雅、扩展性更强的方式重新实现。建议同时阅读一下 [SE-0228](https://github.com/apple/swift-evolution/blob/master/proposals/0228-fix-expressiblebystringinterpolation.md) 这个提案，对比了解下新旧两种实现方式，以及新的实现方式所带来的可扩展性和性能改进。

### 🐎 [唯品会iOS代码覆盖率的应用实践](https://mp.weixin.qq.com/s/mTXMbm0c-nCWZ8v8D1-QAA)
...
### 🐕 [没有AST, IDE中的错误提示、自动补全、重构、语法检查......都玩不转了](https://mp.weixin.qq.com/s/UYzwVRPFas6hwe2U7R0eIg)
