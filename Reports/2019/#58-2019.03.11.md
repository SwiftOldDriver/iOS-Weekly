...
### 🚧 🐎 [Converting Static Libraries to Dynamic Libraries for iOS](https://pewpewthespells.com/blog/convert_static_to_dynamic.html)

本文介绍了静态库转动态库的两种实用教程。

1. 使用 **libtool**,**lipo** 等命令，来分解多架构静态库，将单架构的目标文件使用链接器重新构建单架构的动态库，最后将重构好的多个架构动态库，合并到一个动态库当中
2. 使用 Xcode 自带的动态库生成方式。此方式简单易懂，只需修改对应的配置项和添加对应的二进制文件、头文件、资源，点击构建即可生成。如同我们开发应用一样便捷。

毫无疑问的是我们日常开发中，都会推荐第二种含有 GUI 的方式。报错信息，需要链接库都会有一个良好的提示。如果你想了解动态库和静态库相关的知识，那么第一种方式将是你最好的选择，可以尝试使用一些 **file**,**lipo** 等命令开扩展一下自己的知识广度。
