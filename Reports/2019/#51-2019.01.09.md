...
### 🐎 [记一次重构](http://sindrilin.com/2019/01/05/one_reconstruct.html)

作者以思维导图形式展示了对`重构`中关键词的理解，说明了重构的步骤：业务梳理、明确目标、动手实践、测试回归。并以微视 4.8 中音乐播放器为例，向大家展示重构的过程及踩的一些坑。重构后的播放器主要有三处设计点：KVO 的引用分离、接口和实现的分离、前后台切换的暂停续播，思路与做法可供参考。
