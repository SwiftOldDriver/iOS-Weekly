...
[@JonyFang](https://github.com/JonyFang)：iOS 开发过程中会用到各种缓存，YYCache 也是大家所熟悉的一个缓存框架。由 YYKit 作者 @ibireme 开发，是一个线程安全的高性能缓存框架。其实 YYCache 大家应该都已经很熟悉了，本篇主要介绍的是 YYMemoryCache 和 YYDiskCache 的核心部分源码分析及一些使用过程中的小 Tips，可以拿来借鉴使用。除了本文，也推荐看下其他的几个 YYCache 的源码分析文章。链接放到这里：
