...
@Smallfly：这篇文章揭晓了 [Swift.org](https://www.swift.org/) 首页那只「飞鸟」的诞生过程。作者深入剖析了这个令人印象深刻的动画：它并非视频，而是巧妙地利用 HTML5 Canvas 和 JavaScript 实现的纯代码杰作。
核心亮点在于：
...
@ChengzhiHuang：可以开启国行设备体验 Apple AI 的工具，优势是不用长期关闭 SIP (完成后可重新开启)。原理是通过修改 eligibilityd 沙盒文件 + 锁权限 。需要注意的是，其中关于注入 eligibilityd 相关代码报错的描述存在一些错误，[XcodeLLMEligible](https://github.com/Kyle-Ye/XcodeLLMEligible/) 并未替换系统的 eligibilityd ，仅是在开发调试 eligibilityd 时对齐系统行为所用。
