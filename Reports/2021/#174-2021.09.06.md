...
[@Smallfly](https://github.com/iostalks)：这篇文章比较长，详细的介绍了 58 同城在 Swift 和 Objective-C 混编过程中，编译速度优化的分析与实践。

Swift 和 OC 混编，要实现 Swift 引用 OC 文件，一种方案是添加桥接文件，另一种是采用 Module 化的方式引用，第一种方案有较高的维护成本，58 采用了第二种。

然而，在尝试 Module 化之后，整体的编译耗时比非 Module 化的增加了，这和苹果官方给出的性能数据是不符的。
