...
每个 APP 会由于项目特点以及团队组织结构的区别，在实践中会细节有些许不同，但目的都是明确的：提高可维护性、开发效率。网易新闻业务之间耦合较多，业务之间频繁相互调用接口也有成本；团队没有按业务进行拆分，同时业务之间也不需要隔离。因此网易新闻没有选择通过路由将业务完全隔离，而是将业务拆为两层，顶部的展现层利用路由隔离，下方的业务层可以单项依赖。
