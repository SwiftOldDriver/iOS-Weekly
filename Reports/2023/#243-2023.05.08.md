...
[@老峰](https://github.com/Gesantung)：本文主要介绍了快手接入 Swift 后开启 Clang Module 后所遇到的问题如编译错误、Module 编译慢、PCH 编译优化失效、Module 复用率低 & 过期导致编译失败等，以及对应的解决方案如方案 Clang 关闭 Clang Module ，原理非常简单，主要是打破了接入 Swift 过程中的一些经验“误区”。
