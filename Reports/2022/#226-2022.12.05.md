...
[@水水](https://www.xuyanlan.com/)：作者从收到一个小程序在调用 my.redirectTo() 之后，再调用 my.navigateTo() 时无响应，无法跳转下一个页面的线上问题反馈说起，到排查到是因为一个 WebKit Bug 导致的过程完整的进行了阐述。是一个比较少见的排查难度较高、问题根因较深的问题，文章将排查期间的思考记录下来，希望也可以为读者做参考。作者最后提醒我们：人的固定认知，不一定就是对的~
