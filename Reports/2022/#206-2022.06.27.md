...
[@JimQ](https://github.com/waz0820)：今年的 WWDC 终于带来了在 UITableView/UICollectionView Cells 中渲染 SwiftUI 视图的能力，但并不是可以完全混用了，仅限于 Cells（比如 UITableViewHeaderFooterView 就不能用），需要使用一个新的 UIHostConfiguration 对象 wrap SwiftUI 视图并赋值给 cell.contentConfiguration，Cells 与 SwiftUI 之间要依赖 UIHostConfiguration 的桥接支持。John Sundell 的这篇文章可以带领大家了解更多详情，并充分利用这一新特性。
