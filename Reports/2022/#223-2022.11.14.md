...
继上次 10 月下旬的 Ask Apple 之后，苹果会再继续进行2轮新的Ask Apple活动，一轮是下周，另一轮是12月份。明年 WWDC23 之前，这可能是最后的两轮 Ask Apple 活动了。活动照旧需要提前注册并下载 Slack 。
...
[@莲叔](http://aaaron7.github.io/)：包 Size 是目前的移动开发者都不得不面对的一个问题。近期，Xcode 14 携带了众多新特性，其中一点值得开发者注意的就是将不会默认开启 bitcode，这将会导致潜在的包size增加。文章的作者团队进行了些初步的分析发现很多 APP 在切换使用 Xcode 14打包以后都产生了 30%-60% 的 size 膨胀，这还是很惊人的。文章不仅分析了现状，还对膨胀的原因做了深入的分析。对于面临 Xcode 14 适配的开发者们不容错过。
...
* 本期话题：抖音 iOS 基础技术大揭秘 Vol.02 已完成分享；11 月 14 日，参与新一周的 Ask Apple 活动
