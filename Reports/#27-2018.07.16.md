...
GPUImage的作者实在是太高产了。GPUImage3的shader处理部分完全使用Metal框架重写了。当然GPUImage的pipeline还是和swift2一样，基于Protocol，利用ImageSource和ImageConsumer两个protocol来处理多滤镜的链接问题。
因为Metal的shader不是用glsl写的。用的是C++，所以Brad Larson也用C++重写了一部分shader的代码。
如果研究过GPUImage2的同学看3应该也不会有什么压力。毕竟刨去Metal的内容，GPUImage3和2区别很小。
