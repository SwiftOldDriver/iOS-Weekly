...
另外，本文是近两年来作者关于函数式编程系列文章中的最新一篇。本系列文章同样都是采取引导递进式的行文风格，作者从 Haskhell 这种标准化的、通用纯函数式的编程语言中汲取灵感，将诸多函数式编程的核心概念改造为 Swift 实现，让我们从新的角度去认识 Swift 以及 与 Swift 相关的函数式编程思想，Swift 或 函数式编程爱好者不容错过。
